...
const Todolist_pending = ({tasks,toggleStatus,deleteTask}:{tasks?:typeTodo,toggleStatus:Function,deleteTask:Function}) => {
  const [Mytasks, setMyTasks] = useState<typeTodo[]>([]);
      useEffect(() => {
      const gettaskData = async() => {
        try {
          if (tasks) {
            const data = await tasks.filter((task:typeTodo) => task.status === "Pending")
...
export default Todolist_pending