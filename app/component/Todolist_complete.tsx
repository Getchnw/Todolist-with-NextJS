...
const Todolist_complete = ({tasks,toggleStatus,deleteTask}:{tasks?:typeTodo,toggleStatus:Function,deleteTask:Function}) => {
    const [Mytasks, setMyTasks] = useState<typeTodo[]>([]);
    useEffect(() => {
    const gettaskData = async() => {
      try {
        if (tasks) {
          const data = await tasks.filter((task:typeTodo) => task.status === "Completed")
          await setMyTasks(data);
          console.log(Mytasks);
        } 
      }catch (error){
      console.log(error)
...
export default Todolist_complete